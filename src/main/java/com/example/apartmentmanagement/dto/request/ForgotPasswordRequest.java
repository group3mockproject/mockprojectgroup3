...
public class ForgotPasswordRequest implements Validator {
    ...
    @Pattern(regexp = "^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[@$!%*?&/_])[A-Z][A-Za-z\\d@$!%*/?&]{7,49}$",
            message = "Password must start with an uppercase letter, contain at least one lowercase letter, one number, a special character (@$!%*?&/_), and must be between 8 and 50 characters long!")
